...
function configure_node() {
  ...
  hostname="$(printf "${MASTER_HOSTNAME_FORMAT}" "${node_num}")"
  ...
  if [[ "$node_type" == "arbiter" ]]; then
    local hostname
    # shellcheck disable=SC2059
    hostname="$(printf "${ARBITER_HOSTNAME_FORMAT}" "${node_num}")"
    local ip=$((base_ip + NUM_MASTERS + node_num))
  fi
  if [[ "$node_type" == "worker" ]]; then
    local hostname
    # shellcheck disable=SC2059
    hostname="$(printf "${WORKER_HOSTNAME_FORMAT}" "${node_num}")"
    local ip=$((base_ip + NUM_MASTERS + NUM_ARBITERS + node_num))
  fi
  if [[ "$node_type" == "extraworker" ]]; then
    local hostname
    # shellcheck disable=SC2059
    hostname="$(printf "${EXTRA_WORKER_HOSTNAME_FORMAT}" "${node_num}")"
  ...
  local node_mac
  node_mac=$(sudo virsh dumpxml "$cluster_name" | xmllint --xpath "string(//interface[descendant::source[@bridge = '${BAREMETAL_NETWORK_NAME}']]/mac/@address)" -)
...
function generate_cluster_manifests() {
  ...
  if [[ "$IP_STACK" = "v4v6" ]]; then
     PROVISIONING_HOST_EXTERNAL_IP_DUALSTACK=$(nth_ip "$EXTERNAL_SUBNET_V6" 1)
...
function enable_load_balancer() {
  ...
  if [[ "${AGENT_PLATFORM_TYPE}" == "none" || "${AGENT_PLATFORM_TYPE}" == "external" ]] && [[ "${NUM_MASTERS}" -gt "1" ]]; then
