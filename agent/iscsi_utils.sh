...
function agent_create_iscsi_target() {
    ...
    # rm -rf  /tmp/master-0-0

    # create disks
    sudo qemu-img create  -f raw /tmp/$name 30G

    # Create iqn
    sudo targetcli backstores/fileio create name=$name size=30G file_or_dev=/tmp/$name
    ...
    sudo dd conv=notrunc if=${agent_iso} of=/tmp/${name} status=progress
}
