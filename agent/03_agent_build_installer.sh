...
function patch_openshift_install_version() {
    local res
    res=$(grep -oba ._RELEASE_VERSION_LOCATION_.XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX "${OCP_DIR}/openshift-install")
    local location=${res%%:*}
    ...
    if [[ ! -z ${location} ]]; then
        ...
        printf "%s\0" "$version" | dd of="${OCP_DIR}/openshift-install" bs=1 seek="${location}" conv=notrunc &> /dev/null 
    fi
}

# Copy install binary if built from src
if [ ! -z "$KNI_INSTALL_FROM_GIT" ] && [ -f "$OPENSHIFT_INSTALL_PATH/bin/openshift-install" ]; then
