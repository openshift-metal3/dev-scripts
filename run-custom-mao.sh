...
wait_for_metal3_to_start() {
    ...
    while [ "$(oc --config "${KUBECONFIG}" get pods -n openshift-machine-api | grep -c metal3)" != 1 ]; do sleep 5; done
}
...
check_if_new_mariadb_password_exists() {
    if [ "$(oc --config "${KUBECONFIG}" get secrets --namespace openshift-machine-api | grep -c metal3-mariadb-password)" != 1 ]; then
      exit 1
    else
