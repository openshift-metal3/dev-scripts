...
if [[ -n "$MIRROR_IMAGES" || -z "${IP_STACK:-}" || "$IP_STACK" = "v6" ]]; then
   ...
   if [[ ! -z ${AGENT_E2E_TEST_SCENARIO} ]]; then
       ...
       # For oc-mirror need to do the override
       if [[ "${MIRROR_COMMAND}" == "oc-mirror" ]] && [[ ! "${AGENT_REGISTRY_ON_NODE:-}" == "true"  ]]; then
           ...
           export OPENSHIFT_INSTALL_RELEASE_IMAGE_OVERRIDE="${LOCAL_REGISTRY_DNS_NAME}:${LOCAL_REGISTRY_PORT}/${LOCAL_IMAGE_URL_SUFFIX}:${release_candidate}-x86_64"
       fi
   else
